...
fall back to `$CLAUDE_PROJECT_DIR/rules/payloads.md` (which holds the
...
1. Read `$CLAUDE_PROJECT_DIR/rules/waf-bypass-protocol.md` — the 7-level ladder is the contract.
2. Read the WAF-specific section of `$CLAUDE_PROJECT_DIR/rules/payloads.md` (Cloudflare / Akamai / AWS WAF / F5 / Imperva / Sucuri variants).
...
$CLAUDE_PROJECT_DIR/tools/brain.py record <target> waf-profile
...
for each rung live in `$CLAUDE_PROJECT_DIR/rules/payloads.md`; this is
